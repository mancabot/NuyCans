		...
		try {
			...
			mess = {
				...
				only: {
					...
					public: '[❗] Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup Manca Bot, ketik #darkgroup\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*! ',
...
85. Oʀᴇ ɴᴏ Iᴍᴏᴜᴛᴏ ɢᴀ Kᴏɴɴᴀɴɪ Kᴀᴡᴀɪɪ Wᴀᴋᴇ ɢᴀ Nᴀɪ`, text, tescuk, cr)
				...
				if (!isOwner) return
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					costum( '*MANCA GROUP*\n\n_Yuk masuk kak_\n_sama sama belajar buat bot_\n*https://chat.whatsapp.com/JL2P1ez4crxC9mFIVlmZEf*', text, tescuk, cr)
					break
					case 'fume':
					if (!isOwner) return
					case 'funmenu':
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (!isPremium) return reply(mess.only.premium)
				...
				client.updatePresence(from, Presence.composing) 
 if (isBanned) return reply(mess.only.benned)    
 reply(`sedang diproses 2+ menit, btw yang buat Nuy/owner🗿`)
				if (!isUser) return reply(mess.only.userB)
 ...
 if (isBanned) return reply(mess.only.benned)    
          ...
          reply('Wait..')
                                       ...
                                       if (args[0].startsWith('83815956151')) return reply('Gagal tidak dapat menelpon nomer owner')
...
client.updatePresence(from, Presence.composing) 
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					mentions(`${sew}\n\nTagall by~~ *${pushname2}*\n\n╔══✪〘 Mention All 〙✪══\n╠➥ *Pesan:* `+teks+`╚═〘 MANCA BOT 〙`, members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
	...
	.then(() => {client.sendMessage(from, options, text)})
               ...
               client.updatePresence(from, Presence.composing) 
				...
				//frendowner
				case 'addadmin':
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner MANCA BOT!', id)
                   i = body.slice(8)
                   adminbotnumber.push(i)
                   fs.writeFileSync('./src/admin.json', JSON.stringify(adminbotnumber))
                   reply('Success Menambahkan Admin MANCA BOT!')
                   break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				default:
                  ...
                  if (budy == 'cekprefix') {
                  costum(`*MANCA BOT USING PREFIX [ ${prefix} ]*`, text, tescuk, cr)
                  }
